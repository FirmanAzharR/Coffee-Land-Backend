...
module.exports = {
  getProductModel: (limit, offset, category, search, sort) => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT*FROM product JOIN category ON product.category_id = category.id_category  WHERE product.category_id LIKE '%${category}%' AND product.product_name LIKE '%${search}%' GROUP BY product.product_id ORDER BY ${sort} LIMIT ${limit} OFFSET ${offset}`,
        (error, result) => {
